...
`just` stages everything before building because flakes only see git-tracked files — new files must be at least staged or the build silently ignores them.

## Architecture

This flake emulates [nix-config-modules](https://github.com/chadac/nix-config-modules): instead of writing `nixosConfigurations`/`darwinConfigurations` directly, `flake.nix` runs `lib.evalModules` over `./modules` once per host and merges the results with `lib.recursiveUpdate`.
...
`nixos/` holds legacy NixOS modules imported directly by `modules/default.nix` (`nixos/system`) and `modules/home/default.nix` (`nixos/home`) — new config belongs in `modules/`, not there.

### Neovim

`modules/neovim/lua`, `after/`, and `lazy-lock.json` are symlinked into `~/.config/nvim` with `mkOutOfStoreSymlink`, hardcoded to the repo living at `~/workspace/jimbo`. Lua edits take effect immediately without a rebuild; only `init.lua` and the LSP/tool packages in `default.nix` require a rebuild. Lua is formatted with stylua (`stylua.toml` at repo root).
